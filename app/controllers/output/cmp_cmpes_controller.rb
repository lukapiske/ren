class Output::CmpCmpesController < OutputController
  ...
  def index
    ...
    respond_to do |format|
      format.html
      format.js { render :partial => 'output/cmp_cmpes/list', :object => @datas  }
    end
  end
  def ajax_index
    datas
  ...
  def datas
    if params[:cmp_cmpes]
      ...
      if params[:cmp_cmpes][:country_id].nil?
        session[:country_id] = current_user.country_id
      else
        session[:country_id] = params[:cmp_cmpes][:country_id]
      end

      if params[:admin_user].nil?
        session[:zone] = params[:cmp_cmpes][:zone]
      else
