require File.dirname(__FILE__) + '/../test_helper'

# make sure the secret for request forgery protection is set (views will
# explicitly use the form_authenticity_token method which will fail otherwise)
PurchasesController.request_forgery_protection_options[:secret] = 'test_secret'

class PurchasesControllerTest < ActionController::TestCase

  def test_should_get_index
    get :index
    assert_response :success
    get :index, :format => 'ext_json'
    assert_response :success
    assert_not_nil assigns(:purchases)
  end

  def test_should_get_new
    get :new
    assert_response :success
  end

  def test_should_create_purchase
    assert_difference('Purchase.count') do
      xhr :post, :create, :format => 'ext_json', :purchase => { }
    end

    assert_not_nil flash[:notice]
    assert_response :success
  end

  def test_should_show_purchase
    get :show, :id => purchases(:one).id
    assert_response :success
  end

  def test_should_get_edit
    get :edit, :id => purchases(:one).id
    assert_response :success
  end

  def test_should_update_purchase
    xhr :put, :update, :format => 'ext_json', :id => purchases(:one).id, :purchase => { }
    assert_not_nil flash[:notice]
    assert_response :success
  end

  def test_should_destroy_purchase
    assert_difference('Purchase.count', -1) do
      xhr :delete, :destroy, :id => purchases(:one).id
    end

    assert_response :success
  end
end
